...
[DONE] 14. 폴더 이동 구현
[ ] 9/10 에러났을 때 처리 어떻게 할지, 사이즈에 맞는 옷이 없으면??..
[ ] 9/6 ~ 9/8 5. 디자인 입히기 ⭐⭐⭐⭐(드레스룸에서 아이템보면 좀 예쁘게, 스와이프에서 아아템 사이즈 볼 때 조금 예쁘게)
[ ] 9/9 6. 배포할 때 여러 준비물 준비하기(개인정보처리방침, 서비스이용약관) ⭐⭐⭐⭐
[ ] 하트하고 싫어요 애니메이션
[ ] 개발자 아이디 빼기, Audience 그룹 삭제
[ ] 이벤트 심는거에만 좀 집중하고
[ ] range 에러가 안나게끔, 다시 시도해주세요.
[ ] 피드백 받기

500 에러 페이지로 넘기고 앱 재실행
에러 세팅 => 
Login with KaKao 깨짐
연필 버튼
기본폴더 삭제, 연필, 위쪽 여백이 넘김, (글자 크기 제한)
스와이프 화면 선택 크기가 너무 작다
폴더 버그
MAKE BUTTON 알려줘야 함
룩북에 사러가기 추가안했음
